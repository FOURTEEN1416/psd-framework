...
| C4 | 无监督运动词量化能在连续骨架流上切出与真实边界对齐的行为单元 | SMQ 边界 IoU **0.458±0.049** vs 等段数随机切分 null ≈0.30（≈1.53×，seeds 伪 GT 协议；⚠️ 非滑窗基线——W34 勘误，全仓无滑窗方法臂）；fig3 定性可视化配对 | ✅ 已可写（`reports/p02-smq-iou-eC-seeds-recheck.json`） | §3.2 + §4.3 |
...
| C6 | 半监督自训练以 ≤20% 标注量逼近全监督水平（人类域参照：TCL 10% 标注达 82.7%，全监督 88.6%） | 池精度 **0.691±0.013**（r1 操作点 cov≈35%）+ 首末配对 Δ **+10.69±3.28pp (p=0.030)**（`reports/p04-tcl-results.json`）；⚠️ 实测口径为伪标签池精度而非端到端分类精度——"逼近全监督水平"措辞需按池精度口径收窄，收窄方案留用户终裁（W36 注记） | 🟡 数字已可写、主张措辞待收窄终裁 | §4.3 |
...
| tab3 | 消融表 | 组件开关矩阵 | §5 | 🟡 六行中四行已映射溯源（P0.3/P0.4/W14），两行 PENDING（−自监督预训练等 W39 full 档 / −无监督分割等 W38 滑窗臂）——experiment-skeleton.md §5 |
