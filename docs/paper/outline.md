...
| C7 | ~~不确定性采样主动学习使 100–200 片段人工预算达到 22 类 ≥85%~~（**该主张经用户裁决①降级，正文与摘要不再声明**） | W14 短预算**负结果**：熵采样未显示优势（b=100 随机 +7.9pp / b=200 +7.1pp，3/3 seeds 同向）+ 真实池 softmax 全饱和诊断（margin 均值 100.9 vs 合成域 ≈10.8）；full-budget 待 GPU——`reports/w14-p05-al-efficiency-2026-08-24.md` §3 | 🔻 已裁决①（2026-08-25）：降级"探索性发现"入 §5 分析节；升级待 W23/full-budget 正证据 + 用户再裁 | §5 |
...
- 四要素齐全（背景/方法/结果/结论）+ **≥3 个数值结果**（[RESULT-1] kNN 倍率已实填、[RESULT-2] 22 类精度、⚠️ [RESULT-3] 原"标注预算节省比"因 C7 裁决①降级失据，需重选源——候选：SMQ 边界 IoU 1.53× / 伪标签迭代 +17.9pp / C1 墙钟保守比 ≥3×，归摘要终稿窗口定）。
