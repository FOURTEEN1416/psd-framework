...
| C6 | 半监督自训练以 ≤20% 标注量逼近全监督水平（人类域参照：TCL 10% 标注达 82.7%，全监督 88.6%） | 池精度 **0.691±0.013**（r1 操作点 cov≈35%）+ 首末配对 Δ **+10.69±3.28pp (p=0.030)**（`reports/p04-tcl-results.json`）；**端到端补强（E7，2026-09-04）**：完整管线 13% 标注预算达全监督 **91%**（0.3095±0.0223 vs 0.3393，公开真实层 AK full12），warm vs scratch 端到端 +6pp/macro-F1 2.8×（`reports/p07-endtoend-ak-full12-2026-09-04.json`）——旧"池精度而非端到端"注记由 E7 解除，端到端量化成立；绝对天花板 ~34% 归数据瓶颈（L7）如实 | ✅ 端到端量化成立（E7） | §4.3 E4+E7 |
