...
| R2 | ✅ 已闭环 | **标题过度主张风险**：R19/R20 复核发现弱的是 low-resource 端到端（动物层为负）、强的是解耦+演化准则（C1 实测 6.07×）——与 R2 原预案方向相反。**R20 模拟审稿最优解=从标题移除 "Low-Resource"**（标题词与动物层负结果不匹配是 framing 级拒稿面），卖点迁到已交付新颖核心；low-resource 作设计目标保留摘要/正文/关键词（上下文即时限定）。标题→"A Physics-Semantics Decoupled Framework for Animal Behavior Recognition under Evolving Evaluation Criteria" | ✅ R20 定稿（tag 待更） |
...
| v2.2 | 2026-09-05 | **R20 标题终裁（模拟审稿最优解）**：从标题移除 "Low-Resource"（动物层端到端为负，标题词与结果不匹配=framing 拒稿面），卖点迁至已交付的解耦+演化准则；low-resource 保留摘要/正文/关键词作设计目标；§5"the word low-resource in our title"改"low-resource design goal"（不再依赖标题）。R2 风险闭环。 |
