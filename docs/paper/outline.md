...
| C6 | 半监督自训练以 ≤20% 标注量逼近全监督水平（人类域参照：TCL 10% 标注达 82.7%，全监督 88.6%） | 池精度 **0.691±0.013**（r1 操作点 cov≈35%）+ 首末配对 Δ **+10.69±3.28pp (p=0.030)**（`reports/p04-tcl-results.json`）；**端到端补强（E7，2026-09-04）**：完整管线 13% 标注预算达全监督 **91%**（0.3095±0.0223 vs 0.3393，公开真实层 AK full12），warm vs scratch 端到端 +6pp/macro-F1 2.8×（`reports/p07-endtoend-ak-full12-2026-09-04.json`）——旧"池精度而非端到端"注记由 E7 解除，端到端量化成立；绝对天花板 ~34% 归数据瓶颈（L7）如实；**P0.8 同协议消融（2026-09-04）**：唯一变量替换 warm-start→通用 SSL（AimCLR@InterPet4D），top-1 全预算 warm 占优（+3.6/+7.7/+3.6pp）、macro-F1 混合如实并报、n=3 未达显著——"语义 warm-start 是低预算关键"获得同协议方法对照（`reports/p08-aimclr-arm-2026-09-04.md`） | ✅ 端到端量化成立（E7） | §4.3 E4+E7 |
