...
| v0.8 | 2026-09-12 | **仓库清理登记（历史脚本退出 HEAD）**：用户令"该删的删"。本册及历史报告引用的全部被替代图形脚本共 60 个自 HEAD 移除（figures/scripts/ 57 + 根目录 W17 遗留 3：make_fig1_overview.py / make_fig2_pseudo_label_loop.py / make_fig34_placeholders.py——后者"W22 保留不改"系当时窗口领地纪律，本次清理经用户明确授权）。判定标准=仅保留各图现行正典版（fig1 v13 / fig2 v17 / fig3 v8 / fig4 v6 / fig5 v6 / GA v15）+ 在役资产（2col 双栏×2、GA 候选 c1-c3、fig3b 匹配图）+ 真源与门禁（psd_style / make_common_gates / make_icon_glyphs）。全部被删文件可经 git 历史恢复（删除前最后一次完整存在于 commit `afbef68`）；本册上方所有历史条目指向的旧版本号因此以 git 历史为准。现行正典对照见 `reports/fig-redraw-2026-09-11.md` 与 BOARD 2026-09-11/12 登记。 |
