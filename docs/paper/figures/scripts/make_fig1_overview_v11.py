# -*- coding: utf-8 -*-
r"""fig1 v11 — PSD 框架总览（R30 海洋清风编辑级：无阴影扁平卡+细流线；
坐标拓扑承 v10 不变）。

R30 (diagram-design + figures4papers 联合规范) 要素对照：
- 容器=极浅柔色满块（海洋清风板提亮，无边）；
- 模块=白色扁平卡（无阴影），标题 DEEP 系粗体+灰副文两级文字；
- 入口/出口模块=DEEP 满色白字（视觉锚点，每图焦点 ≤2 处纪律）；
- 层间接口带=浅绿桥（IFACE，R30 新语义：桥=冷冷之间的浅绿）；
- 数据流=1.6pt 细流线（编辑级线宽，物理段青/语义段橙/接口深灰黑）。
"""
import matplotlib
matplotlib.use("Agg")
...
def wcard(x, y, w, h, title, sub=None, deep="#333333", z=5,
    ...
    """白色模块卡 + DEEP 标题(+灰副文)。R30: card 无阴影。"""
...
psd_style.card(ax, 2, 14, 44, 62, fill=PT, edge=None, rounding=2.0, z=2)
ax.text(24, 73.0, r"Physics layer $\Phi$  (frozen)", ha="center", va="center",
        fontsize=7.2, fontweight="bold", color=PD, zorder=6)
psd_style.card(ax, 64, 14, 44, 62, fill=ST_, edge=None, rounding=2.0, z=2)
...
psd_style.flowline(ax, 33.5, 68.3, 52.3, 68.3, color=PD)   # 降线避让容器标题(承 v9)
...
psd_style.flowline(ax, 84, 8.3, 86.0, 16.6, color=SD_, lw=1.3, ls=(0, (3, 2)), head=9)
...
print("fig1 v11 (R30 ocean editorial: flat cards + thin flowlines + mint iface) saved")
