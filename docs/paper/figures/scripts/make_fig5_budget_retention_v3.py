...
v2 (R24 图表专项): 顶部点簇净空≥3pt——PanAf 位移 12.5->13.2, synthetic-offset
位移至 8.0 且换紫色拉开明度(原与 NTU 系同深板岩色仅形状可分); 全标记加白描边。
...
pts.append((13.9, 100 * v1["top1_mean"] / v1_full, 100 * v1["top1_std"] / v1_full,
            "public-real v1", "o", psd_style.PHYS_EDGE))
...
for spc, n_anc, x_disp in ((2, 14, 5.5), (4, 28, 11.5)):
    ...
    pts.append((x_disp, 100 * s["top1_mean"] / v2_full, 100 * s["top1_std"] / v2_full,
                "public-real v2", "s", psd_style.SEM_EDGE))
...
pts.append((10.0, ret, err, "human benchmark (NTU60)", "D", psd_style.GRAY_2))
...
pts.append((16.5, ret120, err120, "human benchmark (NTU120, HRNet 2D)", "v", psd_style.GRAY_3))
tbucf = p5b_ucf["b_arms"]
retucf = 100 * np.mean(tbucf) / p5b_ucf["full_ref"]
errucf = 100 * np.std(tbucf, ddof=1) / p5b_ucf["full_ref"]
pts.append((8.8, retucf, errucf, "independent benchmark (UCF101, HRNet 2D)", "P", psd_style.GRAY_1))
...
pts.append((13.2, retpan, errpan, "animal public benchmark (PanAf500)", "X", psd_style.GRAY_4))
...
pts.append((8.0, 100 * syn["mean"] / syn_full, 100 * syn["std"] / syn_full,
            "synthetic-offset", "^", psd_style.VIOLET))
