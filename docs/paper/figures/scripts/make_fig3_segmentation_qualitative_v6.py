# -*- coding: utf-8 -*-
"""fig3 v3 — SMQ 分割边界定性图（R24 图表专项精修, 2026-09-10）。

v3 修复 judge 三处: ①baseline 数值标签右移+白底避让相邻组青柱(原灰字叠青柱);
②柱状图图例并入图区右上(原悬在两栏之间归属含混); ③时间轴补 frame index 轴名、
ytick 按轨道配色加粗(橙=Pseudo-GT/青=SMQ, 色义直接绑定)。
...
BAND_GRAY, BASE_EDGE = psd_style.GRAY_FILL, psd_style.GRAY_EDGE   # R28: null 参照=中性灰
...
gs = fig.add_gridspec(3, 1, height_ratios=[1, 1, 1.15], hspace=0.75)
...
axi.legend(loc="upper right", ncol=2, frameon=False, fontsize=5.5, bbox_to_anchor=(1.0, -0.16))
...
print("fig3 v4 written"); print("written:", pdf_path)
for i in ids:
    ep = episodes[i]
