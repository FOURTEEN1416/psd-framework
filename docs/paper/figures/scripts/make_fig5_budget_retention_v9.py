# -*- coding: utf-8 -*-
"""fig5 v9 — 预算-保留率跨层汇总图 · 正典（风格方向 **C+ 强化版** + 尺度修复）。
...
━━ C+ 相对 v8 的收紧项 ━━
1. **笔触更细**：spines 0.6 -> 0.4；tick width 0.6 -> 0.4、length 2.2 -> 1.8；
   网格 0.4 -> 0.35；误差棒 elinewidth 0.8 -> 0.6、capsize 2.4 -> 2.0、capthick 0.6；
   marker 6.0 -> 5.2、edge 0.9 -> 0.6；tonal ring 8.6 -> 7.6；100% 基线 0.9 -> 0.6；
   引线 0.5 -> 0.4。
2. **字号阶梯更大**：轴名 6.8 -> 7.2；刻度 5.6 -> 5.8；7 个直标 5.6 -> 5.8；
   own full budget 注记 5.5 -> 5.6；预注册带注记 5.5 -> 5.6。下限 5.5pt。
3. **留白**：画布 2.75 -> 2.95in。因 ylim(0,108) 不变，加高使 1 数据单位变高
   => 同字号文字占更少数据单位 => 标签与 marker/误差棒的净空**单调改善**，
   故各标签数据坐标无需重算（已由 G2/G3 + 本地引线自检复验）。
   （预注册带注记横跨 x∈[4.40,7.5] 的左侧窗口，字号 5.6 时实测仍止于 7.52
   < synthetic-offset 环左缘 7.68，净空保持。）
...
# ---- 固定画布：宽 3.42in == 版心 246.24pt；高 2.95in（C+ 留白） ----
FIG_W, FIG_H = 3.42, 2.95
fig, ax = plt.subplots(figsize=(FIG_W, FIG_H))
fig.patch.set_facecolor("white"); ax.set_facecolor("white")
# 去 tight 后由本行显式定版心（保证 ylabel/ticklabel/xlabel 全落在画布内且四周留白均匀）
fig.subplots_adjust(left=0.118, right=0.982, bottom=0.125, top=0.985)
...
for frac, r, e, tier, mk, col in pts:
    ...
    ax.errorbar(frac, r, yerr=e, marker=mk, color=col, markersize=MARKER_SIZE,
                ...
                markeredgewidth=0.6, elinewidth=0.6, capsize=2.0, capthick=0.6,
                zorder=3)
...
ax.set_xlabel("Annotation budget (% of the tier's full-labeled pool, log)", fontsize=7.2)
ax.set_ylabel("Retention of own full-budget top-1 (%)", fontsize=7.2)
ax.grid(True, axis="y", color="#EFEFEF", linewidth=0.35, zorder=0)
...
for tier, (text, x, y, ha, va, tcol) in DIRECT.items():
    t = ax.text(x, y, text, fontsize=5.8, color=tcol, ha=ha, va=va,
                fontweight="normal", zorder=4)
    label_texts.append(t)
for tier, spec in LEADER_SPEC.items():
    ax.plot([p[0] for p in spec["path"]], [p[1] for p in spec["path"]],
            color="#BFBFBF", linewidth=0.4, solid_capstyle="butt", zorder=2)
    t = ax.text(spec["x"], spec["y"], spec["text"], fontsize=5.8, color=spec["tcol"],
...
ax.axhline(100, color=GRID_GRAY, linewidth=0.6, zorder=1)
t_own = ax.text(29.6, 99.4, "own full budget", fontsize=5.6, color="#767676",
...
gates.gate_labels(fig, ax, label_texts, name="fig5-v9")
gates.gate_whisker_texts(fig, ax, label_texts, point_errs, name="fig5-v9")
_assert_leaders_clean(fig, ax, leaders, point_errs)
gates.gate_typography(fig, min_pt=5.5, name="fig5-v9")
gates.gate_marks_in_axes(fig, name="fig5-v9")
...
print("fig5 v9 (direction C+ intensified, fixed canvas) saved")
