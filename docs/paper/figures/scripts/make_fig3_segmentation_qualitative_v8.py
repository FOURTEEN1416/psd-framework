# -*- coding: utf-8 -*-
"""fig3 v8 — SMQ 分割边界定性图（R43b 顶刊级冲刺 · Nature 系面板字母惯例）。

相对 v7 的顶刊化精修（拓扑/底轨/数据源承 v7 不动）：
1. 面板字母 a/b/c（Nature 惯例：左上角粗体小写、轴外）——caption 的
   "upper band / lower band / lower panel" 叙述与 a/b/c 面板序天然对应，
   字母为加法项，caption 零改动；
2. 网格再减淡一档（0.5→0.4/0.45）、轴线 0.7→0.65、ticks 0.65/2.4；
3. 轨道加高 0.62→0.66（印刷存在感）。
...
print("fig3 v8 written"); print("written:", pdf_path)
for i in ids:
    ep = episodes[i]
