# -*- coding: utf-8 -*-
r"""fig2 双栏版（v17_2col）— 伪标签循环双行管线 7in 资产（任务包 A 第三轮 · 候选 c）。

新资产 fig2_pseudo_label_loop_2col.pdf/png（单栏版 v16 不动，是否采用由用户裁决）。
相对 v16：坐标系与拓扑逐点同构（0-100 × 25-74，双行拓扑 / 贴盒回路轨道 /
write-back 正交肘线 / 单行三键图例 / ①-④ 编号 chip），画布 3.42→7.0in，解锁：
1. 站盒微图标（tag/stack/circular-arrow/two-arrows/person/check 与 hub 圆柱）；
2. 字号上调一档（站名 5.9→7.2pt，图例 5.6→6.4pt）。
色板/语义/拓扑承 psd_style 与 v16 原样；新资产未入 main.tex，caption 零耦合。
"""
import matplotlib
matplotlib.use("Agg")
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.patches import FancyArrowPatch, Circle
from pathlib import Path

ROOT = Path(__file__).resolve().parents[4]
OUT = ROOT / "docs" / "paper" / "figures"

import sys, os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import psd_style
import make_icon_glyphs as ic
psd_style.apply_style()
PAPER = "#FFFFFF"
INK = psd_style.INK
ACCENT = psd_style.SEM_DEEP
ACCENT_MID = psd_style.SEM_EDGE
HUMAN = psd_style.HUMAN_DEEP
HUMAN_MID = psd_style.HUMAN_EDGE
SOFT = "#555555"

fig, ax = plt.subplots(figsize=(7.0, 2.75))
ax.set_xlim(0, 100); ax.set_ylim(25, 74)
ax.axis("off")

ST_W, ST_H = 19.5, 8.2
HUB_X, HUB_Y = 61.0, 40.0
HUB_W, HUB_H = 17.0, 10.0

ROW1_Y = 62.0
S = {
    "assign":  (11.0, ROW1_Y),
    "pool":    (36.5, ROW1_Y),
    "upd":     (62.0, ROW1_Y),
    "reest":   (87.5, ROW1_Y),
    "al":      (11.0, 40.0),
    "verified":(36.5, 40.0),
}
NAMES = {
    "assign": "Assign proposals", "pool": "Label pool",
    "upd": "Update $\\Omega$", "reest": "Re-estimate $P$",
    "al": "AL queue", "verified": "Verified seeds",
}
SUBS = {
    "assign": "conf. $\\kappa$", "pool": "conf. pass",
    "upd": "seeds $\\cup$ pool", "reest": "each round",
    "al": "human, $\\leq$B", "verified": "queue output",
}
GLYPHS = {
    "assign": ic.glyph_tag, "pool": ic.glyph_layers,
    "upd": ic.glyph_circular_arrow, "reest": ic.glyph_arrows_two,
    "al": ic.glyph_person, "verified": ic.glyph_check,
}
FOCAL = "pool"
MANUAL = {"al", "verified"}

def edge_pt(c, target, hw=ST_W / 2, hh=ST_H / 2):
    v = np.array([target[0] - c[0], target[1] - c[1]], float)
    L = np.linalg.norm(v); v = v / L
    t = np.inf
    if abs(v[0]) > 1e-9: t = min(t, hw / abs(v[0]))
    if abs(v[1]) > 1e-9: t = min(t, hh / abs(v[1]))
    return (c[0] + v[0] * t, c[1] + v[1] * t)

def seg(a, b, label=None, lp=0.5, ldy=2.2, ls="solid", lw=1.9, color=SOFT,
        a_box=True, b_box=True):
    ca = S[a] if a_box else a
    cb = S[b] if b_box else b
    st = edge_pt(ca, cb) if a_box else a
    en = edge_pt(cb, ca) if b_box else b
    psd_style.flowline(ax, st[0], st[1], en[0], en[1], color=color, lw=lw,
                       ls=ls, head=12, z=2)
    if label:
        mx = st[0] + (en[0] - st[0]) * lp
        my = st[1] + (en[1] - st[1]) * lp + ldy
        ax.text(mx, my, label, fontsize=7.0, color="#444444", ha="center",
                va="center", zorder=5)

# ---- 上排自动环 + 贴盒回路轨道 ----
seg("assign", "pool", label=r"$\kappa \geq \tau$", ldy=-2.2)
seg("pool", "upd")
seg("upd", "reest")
RAIL_Y = 69.5
ax.plot([87.5, 87.5], [66.3, RAIL_Y], color=SOFT, lw=1.9, zorder=2,
        solid_capstyle="round")
ax.plot([87.5, 11.0], [RAIL_Y, RAIL_Y], color=SOFT, lw=1.9, zorder=2,
        solid_capstyle="round")
ax.add_patch(FancyArrowPatch((11.0, RAIL_Y), (11.0, 66.5), arrowstyle="-|>",
             mutation_scale=12, color=SOFT, lw=1.9, zorder=2))
ax.text(49.25, 71.6, "next round", fontsize=7.0, color="#444444", ha="center",
        va="center")

# ---- 下排人工分支 ----
seg("assign", "al")
ax.text(13.4, 51.0, r"$\kappa < \tau$", fontsize=7.0, color="#444444",
        ha="left", va="center")
seg("al", "verified")
seg("verified", (HUB_X - HUB_W / 2 - 0.3, HUB_Y), a_box=True, b_box=False)

# ---- write-back 正交肘线 ----
psd_style.flowline(ax, 62.0, 57.7, 62.0, HUB_Y + HUB_H / 2 + 0.3, color="#777777",
                   lw=1.3, ls=(0, (4, 3)), head=10, z=2)
ax.plot([87.5, 87.5], [57.7, HUB_Y], color="#777777", lw=1.3, ls=(0, (4, 3)),
        zorder=2, solid_capstyle="round")
psd_style.flowline(ax, 87.5, HUB_Y, HUB_X + HUB_W / 2 + 0.3, HUB_Y, color="#777777",
                   lw=1.3, ls=(0, (4, 3)), head=10, z=2)
ax.text(64.0, 51.5, "write-back", fontsize=6.4, color="#666666", ha="left",
        va="center")

# ---- hub(墨黑满块白字 + 圆柱微图标) ----
psd_style.card(ax, HUB_X - HUB_W / 2, HUB_Y - HUB_H / 2, HUB_W, HUB_H, fill=INK,
               edge=None, rounding=1.6, z=6)
ic.glyph_cylinder(ax, HUB_X - HUB_W / 2 + 2.6, HUB_Y, HUB_H * 0.52, "white", lw=0.8)
ax.text(HUB_X + 1.2, HUB_Y + 2.8, "Prototypes $P$", ha="center", va="center",
        fontsize=6.8, color=PAPER, fontweight="bold", zorder=8)
ax.text(HUB_X + 1.2, HUB_Y + 0.0, "classifier $\\Omega$", ha="center", va="center",
        fontsize=6.8, color=PAPER, zorder=8)
ax.text(HUB_X + 1.2, HUB_Y - 2.8, "anchors $A$", ha="center", va="center",
        fontsize=6.8, color=PAPER, zorder=8)

# ---- 站盒（微图标居左，文字右移） ----
for k, (xk, yk) in S.items():
    manual = k in MANUAL
    deep = HUMAN if manual else ACCENT
    mid = HUMAN_MID if manual else ACCENT_MID
    gx = xk - ST_W / 2 + 2.8
    if k == FOCAL:
        psd_style.card(ax, xk - ST_W / 2, yk - ST_H / 2, ST_W, ST_H, fill=ACCENT,
                       edge=None, rounding=1.6, z=4)
        GLYPHS[k](ax, gx, yk, ST_H * 0.50, "white", lw=0.8)
        ax.text(xk + 1.2, yk + 1.1, NAMES[k], ha="center", va="center",
                fontsize=7.2, color="white", fontweight="bold", zorder=6)
        ax.text(xk + 1.2, yk - 1.9, SUBS[k], ha="center", va="center",
                fontsize=6.6, color="white", alpha=0.92, zorder=6)
    else:
        psd_style.card(ax, xk - ST_W / 2, yk - ST_H / 2, ST_W, ST_H, fill="white",
                       edge=mid, lw=1.1, rounding=1.6, z=4)
        GLYPHS[k](ax, gx, yk, ST_H * 0.50, deep, lw=0.8)
        ax.text(xk + 1.2, yk + 1.1, NAMES[k], ha="center", va="center",
                fontsize=7.2, color=deep, fontweight="bold", zorder=6)
        ax.text(xk + 1.2, yk - 1.9, SUBS[k], ha="center", va="center",
                fontsize=6.6, color="#555555", zorder=6)

# ---- 自动环步骤号 chip ①-④ ----
for i, k in enumerate(("assign", "pool", "upd", "reest"), 1):
    xk, yk = S[k]
    cx, cy = xk - ST_W / 2 + 1.2, yk + ST_H / 2 + 1.2
    ax.add_patch(Circle((cx, cy), 1.5, facecolor="white", edgecolor=ACCENT_MID,
                        lw=0.9, zorder=7))
    ax.text(cx, cy, str(i), ha="center", va="center", fontsize=6.2,
            color=ACCENT, fontweight="bold", zorder=8)

# ---- 图例(底部单行三键) ----
LY = 30.0
psd_style.card(ax, 2.5, LY - 1.4, 4.8, 2.8, fill="white", edge=ACCENT_MID, lw=1.1,
               rounding=0.6, z=8)
ax.text(9.1, LY, "automated step", fontsize=6.6, color=INK, va="center")
psd_style.card(ax, 27.0, LY - 1.4, 4.8, 2.8, fill="white", edge=HUMAN_MID, lw=1.1,
               rounding=0.6, z=8)
ax.text(33.6, LY, "human-in-the-loop step", fontsize=6.6, color=INK, va="center")
ax.plot([63.0, 68.5], [LY, LY], color="#777777", lw=1.3, linestyle=(0, (4, 3)))
ax.text(70.0, LY, "write-back to state", fontsize=6.6, color=INK, va="center")

fig.subplots_adjust(left=0.005, right=0.995, top=0.995, bottom=0.005)
pdf_path = OUT / "fig2_pseudo_label_loop_2col.pdf"
png_path = OUT / "fig2_pseudo_label_loop_2col.png"
fig.savefig(pdf_path, bbox_inches="tight", pad_inches=0.01)
fig.savefig(png_path, dpi=600, bbox_inches="tight", pad_inches=0.01)

# ---- 出图门禁 ----
import make_common_gates as gates
gates.gate_print_robustness(
    "fig2-2col", {"automated": psd_style.SEM_EDGE, "human": psd_style.HUMAN_EDGE},
    redundancy="card fill/border + legend keys + glyph icons")
gates.gate_labels(fig, ax, list(ax.texts), name="fig2-2col")
gates.gate_pdf(pdf_path, max_w_pt=514)
print("fig2 v17_2col (7in + station micro-icons) saved:", pdf_path)
