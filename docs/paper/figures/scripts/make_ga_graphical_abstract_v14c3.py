# -*- coding: utf-8 -*-
r"""GA 候选 C3（v14c3）— "双行"构图：上行 Φ→Ω 解耦对，下行全宽保留率条。

候选菜单 a（任务包 A 第三轮）。与 v13 的差异：
...
语义色、531×131pt。输出候选文件 *_c3.*，不覆盖在位 GA。
"""
...
pdf_path = OUT / "fig_ga_graphical_abstract_c3.pdf"
png_path = OUT / "fig_ga_graphical_abstract_c3.png"
fig.savefig(pdf_path, bbox_inches="tight", pad_inches=0)
fig.savefig(png_path, dpi=600, bbox_inches="tight", pad_inches=0)
plt.close(fig)

assert pdf_path.is_file() and png_path.is_file(), "GA c3 outputs missing"

import make_common_gates as gates
gates.gate_print_robustness(
    "GA-c3", {"NTU60": psd_style.S_NTU60, "NTU120": psd_style.S_NTU120,
