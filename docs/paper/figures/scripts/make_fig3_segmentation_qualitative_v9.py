# -*- coding: utf-8 -*-
"""fig3 v9 — SMQ 分割边界定性图（正典 · 风格方向 C + 尺度修复）。

承 v8 全部构图（双 episode 时间轴 + 四 episode IoU 聚合面板、面板字母 a/b/c、
灰底轨、直标值、网格/笔触），**图内全部文字与数据口径逐字承 v8，零新增零删除**。

数据源 = reports/p02-smq-iou-eC-seeds-recheck.json（E-C 端到端 K=8, epoch-30；
aggregate 0.4577±0.0488 与 caption 逐位一致），维持绑定。

本版唯一改动族：
1. 尺度修复（wt/figB 诊断头号根因）：v8 用 `bbox_inches="tight"` 保存，
   MediaBox=墨迹框 233.65x207.21pt，被 LaTeX 按 \\linewidth 装入时放大
   246.24/233.65 = **1.054 倍**（fig3 是六图中第二严重的拉伸）。本版固定画布
   3.42x3.03in，MediaBox 严格 246.24x218.16pt=版心宽，装入系数 1.000，
   版面占用 218.16pt 与 v8 的 218.39pt 逐点持平（不引发浮动回退）。
2. 去 tight 后的显式版面：gridspec 显式边距（left=0.155/right=0.985/top=0.93/
   bottom=0.06），使 y 轴彩字标签、轴外 a/b/c 面板字母、底排 x 刻度全部落在
   画布内（G5 画布裁切断言实证）。新轴宽 2.84in > v8 的 2.65in，段内标签
   适配更宽松（标签集合与 v8 完全一致：draw_track 的 ax_w_in 估算口径未动）。
3. 字号/色板/线宽/网格/面板字母尺寸全部承 v8（最小 5.5pt，本就合规）。

caption 约束逐词核对（04-experiments.tex \\label{fig:segqual}）：
upper band=seed pseudo-GT(orange) / lower band=SMQ pred(teal)、lower panel=
四 episode IoU 聚合 vs random-cut null、agg 0.4577±0.0488、4/4 > baseline——全部保持。
"""
...
# 标签宽度估算口径承 v8（ax_w_in=3.05 不变 → 段内标签集合与 v8 完全一致）
...
# ---- 固定画布：宽 3.42in == 版心 246.24pt；高 3.03in ⇒ 版面占用与 v8 持平 ----
FIG_W, FIG_H = 3.42, 3.03
fig = plt.figure(figsize=(FIG_W, FIG_H))
fig.patch.set_facecolor("white")
gs = fig.add_gridspec(3, 1, height_ratios=[1, 1, 1.15], hspace=0.62,
                      left=0.155, right=0.972, top=0.93, bottom=0.06)
...
axi.tick_params(labelsize=5.5, width=0.65, length=2.4)
axi.set_ylim(0, 0.86)
axi.grid(True, axis="y", color=GRID_GRAY, linewidth=0.45)
axi.set_axisbelow(True)
for side in ("top", "right"):
    axi.spines[side].set_visible(False)
for side in ("left", "bottom"):
    axi.spines[side].set_linewidth(0.65)
axi.legend(loc="upper right", ncol=2, frameon=False, fontsize=5.8)
axi.text(-0.13, 1.10, PANEL_LETTERS[2], transform=axi.transAxes,
         fontsize=8, fontweight="bold", ha="left", va="bottom", color=INK)
...
print("fig3 v9 (fixed-canvas scale fix) saved")
for i in ids:
    ep = episodes[i]
...
gates.gate_print_robustness(
    "fig3-v9", {"pseudo-GT band": ORANGE_DARK, "SMQ pred band": CYAN_DARK},
    redundancy="upper/lower band position + colored axis labels + bar hatch")
gates.gate_typography(fig, min_pt=5.5, name="fig3-v9")
gates.gate_marks_in_axes(fig, name="fig3-v9")
