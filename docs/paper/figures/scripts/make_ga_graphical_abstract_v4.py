...
规格: ≤531×131 pt ≈ 7.375×1.82 in,印刷尺寸 1:1 铁律(figsize 直接落)，
  ...
  ③ 右侧保留率微条:NTU60 90.7 / NTU120 88.9 (10% labels,各层自身全预算参照)
    + 犬科层如实边界(13%-label 预算保留率 28.9%,条上标数并注绝对精度对 chance,
    修正 R24 judge 指出的量纲混读; NTU 条灰系配色与 fig5 对齐)。
...
rows = [
    ("NTU60", 90.7, False, psd_style.S_NTU60),
    ("NTU120", 88.9, False, psd_style.S_NTU120),
    ("canine", 28.9, True, psd_style.GRAY_FILL),   # E7 v1 @13%-label 预算: 保留率 28.9%, 绝对精度近随机
]
ax.text((PANEL_X0 + 99) / 2, 21.0, "budget retention", ha="center", va="center",
        fontsize=6.3, fontweight="bold", color=C_TEXT)
ys = (15.6, 10.8, 6.0)
BAR_H = 3.0
for (label, val, is_boundary, bcol), y in zip(rows, ys):
    ax.text(BAR_X0 - 1.2, y + BAR_H / 2, label, ha="right", va="center",
            fontsize=5.8, color=C_TEXT)
    w = BAR_MAX * val / 100.0
    if is_boundary:
        ax.add_patch(Rectangle((BAR_X0, y), w, BAR_H, facecolor=bcol,
                               edgecolor=C_SEM_EDGE, linewidth=0.9, hatch="///"))
        ax.text(BAR_X0 + w + 1.0, y + BAR_H / 2, f"{val:.1f}%",
                ha="left", va="center", fontsize=5.4, color=C_SEM_EDGE)
    else:
        ax.add_patch(Rectangle((BAR_X0, y), w, BAR_H, facecolor=bcol,
