# -*- coding: utf-8 -*-
r"""fig2 v12 — 伪标签循环双行管线（R29 顶刊语言：白卡 DEEP 层级文字+焦点 DEEP 满块+
粗圆头流线；拓扑承 v11）。automated=红系，human=兰紫系，hub=黑。"""
import matplotlib
matplotlib.use("Agg")
import numpy as np
...
PAPER = "#FFFFFF"
INK = psd_style.INK
ACCENT = psd_style.SEM_DEEP          # automated 深红
ACCENT_TINT = psd_style.SEM_TINT     # 焦点块柔红填充
HUMAN = psd_style.HUMAN_DEEP         # human 深兰紫
SOFT = "#555555"                      # 流线深灰
...
def seg(a, b, label=None, lp=0.5, ldy=2.2, ls="solid", lw=2.0, color=SOFT,
...
ax.plot([87.5, 87.5], [70.3, 82.0], color=SOFT, lw=2.0, zorder=2, solid_capstyle="round")
ax.plot([87.5, 11.0], [82.0, 82.0], color=SOFT, lw=2.0, zorder=2, solid_capstyle="round")
ax.add_patch(FancyArrowPatch((11.0, 82.0), (11.0, 70.5), arrowstyle="-|>",
             mutation_scale=11, color=SOFT, lw=2.0, zorder=2))
...
for k, (xk, yk) in S.items():
    ...
    if k == FOCAL:
        psd_style.card(ax, xk-wk/2, yk-ST_H/2, wk, ST_H, fill=ACCENT, edge=None,
                       rounding=1.6, dx=0.45, dy=-0.75, z=4)
        ...
        psd_style.card(ax, xk-wk/2, yk-ST_H/2, wk, ST_H, fill="white", edge=None,
                       rounding=1.6, dx=0.45, dy=-0.75, z=4)
        ax.text(xk, yk+1.1, NAMES[k], ha="center", va="center", fontsize=5.9,
...
# ---- 图例(底部左角) ----
LX = 2.0; LY = 24.0
for i, (fc, txt, tc) in enumerate([
        (ACCENT, "automated step", "white"),
        (psd_style.HUMAN_TINT, "human-in-the-loop step", HUMAN)]):
    y = LY + (1-i)*5.5
    psd_style.card(ax, LX, y-1.4, 4.6, 2.8, fill=fc, edge=None,
                   rounding=0.6, dx=0.3, dy=-0.5, z=8)
    ax.text(LX+6.4, y, txt, fontsize=5.6, color=INK, va="center")
ax.plot([62, 67.5], [24.6, 24.6], color="#777777", lw=1.3, linestyle=(0, (4, 3)))
...
print("fig2 v12 (R29 journal-style) saved")
