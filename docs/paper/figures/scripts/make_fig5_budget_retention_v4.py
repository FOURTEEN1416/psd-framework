...
pts.append((10.0, ret, err, "human benchmark (NTU60)", "D", psd_style.S_NTU60))
...
pts.append((16.5, ret120, err120, "human benchmark (NTU120, HRNet 2D)", "v", psd_style.S_NTU120))
tbucf = p5b_ucf["b_arms"]
retucf = 100 * np.mean(tbucf) / p5b_ucf["full_ref"]
errucf = 100 * np.std(tbucf, ddof=1) / p5b_ucf["full_ref"]
pts.append((8.8, retucf, errucf, "independent benchmark (UCF101, HRNet 2D)", "P", psd_style.S_UCF))
...
pts.append((13.2, retpan, errpan, "animal public benchmark (PanAf500)", "X", psd_style.S_PANAF))
