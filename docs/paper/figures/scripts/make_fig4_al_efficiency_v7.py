# -*- coding: utf-8 -*-
"""fig4 v7 — 主动学习效率曲线（正典 · 尺度修复 + 高度上限合规）。

承 v6 全部构图（双面板 sharex、图级单行共享图例、面板内负结果注记、
...
数据：reports/p05-al-efficiency-{short-2026-08-24, warmstart-short-2026-08-25}.json（逐位承 v6）。
...
def panel(ax, curves, title, note, note_xy, tag):
    ...
    for arm, style in SERIES.items():
        ...
        ax.errorbar(xs, means, yerr=stds, color=style["color"],
                    marker=style["marker"], markersize=5.6, markerfacecolor=style["mfc"],
                    markeredgecolor=style["color"], markeredgewidth=1.0, linewidth=1.4,
                    linestyle=style["ls"], elinewidth=0.9, capsize=3,
...
fig.subplots_adjust(left=0.125, right=0.985, top=0.90, bottom=0.085, hspace=0.32)

# ---- 门禁（G1 + G3 双面板 + G5 + G6 + G4 高度上限） ----
import make_common_gates as gates
gates.gate_print_robustness(
    "fig4-v7", {"uncertainty (proposed)": ENT_COLOR, "random": RND_COLOR},
    redundancy="marker shape (o/s) + linestyle (solid/dashed) + legend")
gates.gate_whisker_texts(fig, axes[0], texts_a, PANEL_ERRS["a"], name="fig4-v7-a")
gates.gate_whisker_texts(fig, axes[1], texts_b, PANEL_ERRS["b"], name="fig4-v7-b")
...
gates.gate_typography(fig, min_pt=5.5, name="fig4-v7")
gates.gate_marks_in_axes(fig, name="fig4-v7")
...
print("fig4 v7 (fixed-canvas scale fix, height 3.66in compliant) saved")
