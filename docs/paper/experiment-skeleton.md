...
> ⚠️ **治理注记（2026-09-10 真源交割，用户裁决）**: 本文件的正文装配 truth 已正式移交 `docs/paper/latex/sections/` 各节 tex——R22 之后各轮（fixfull/MEXP/R24/R25）全部直改 tex，本 md 从未回同步；本文件**冻结为历史设计稿**，不得作为修订、引用或数字对账依据。本文件的实验协议/结论台账记录（如 R4 终判行）不受影响，仅撤销“正文装配 truth”身份。
...
| v2.8 | 2026-09-07 | **第二梯队双补强（三梯队推进轮）**：①**SSL 基线跨域两点**——Amendment 1(UCF101) MT 14.17%±0.87 vs PSD 15.40%±1.89 = +1.2pp 8/10 胜 p=0.047(校正不显著,如实)，(a) 臂 14.04% 逐位一致自检=特征同源证明；两点图景=参考强域 SSL 劣势 20.7pp/弱域 1.2pp n.s.，梯度机制在外部基线轴再现（E9/E9c 段入文）；cover letter v0.2（八协议+SSL 牌+FAILS 牌）+审稿人池更新（AimCLR COI 删行/SSL 线新增）+presubmit 三检查全绿。②**子集重抽方差诊断**（post-hoc,非预注册终点）：NTU60 (a) 臂 4 重抽种子(43-46)= 65.62%±0.48，原抽 66.05% 在一个标准差内——E9"单次抽样"披露缺口量化入文，解耦臂子集敏感性如实声明未测。③NTU 截图+官方 §7 ack 语录+Funding 零资助（上一批）。commit 86c5371→本批。 |
| v2.7 | 2026-09-07 | **R23 三审轮+SSL 基线轮（论文完善阶段收官）**：R23a 交叉审稿 4 发现全修（liu2020ntu 补引=NTU 许可§7 强制项/intro ≥3× 限定/GA 90.7/L12 标题）；R23b 对抗审稿 15 发现全修（**S1 假出处=TRANS-001 精度"预测命中"实为 full 档反转**,三处改正；摘要补真域成本失败；E9 系列打印 Holm 校正值；synthetic-offset 定义入 §4.1；post-hoc 标注；E7c 单次运行口径）；R23c 恶意审稿 8 发现全修（**Holm-6 统一族** 3tier×2stat 校正值打印+结论族不变声明；方向句统一"did not materialize"；**梯度可证伪化**≥70%参考+<85%保留=破坏条件+K9 试点=样本外检验；E9d 样本外声明；PanAf 线性头承载如实；E7c 粒度归因与梯度统一；E7b 同批披露入 §4.2；crossover 与 E6-real 调和）。**压制项#1 消灭：PSD-SSL-BASE-001 冻结执行——Mean-Teacher 46.79%±0.61 vs PSD 67.53%±0.24，+20.7pp 10/10 胜（W p=0.002 校正 0.008）**,mt τ=0.95 仅收 15.8k/36.1k 池=机制一致解释入文+L11 harness 限定扩展；五臂 crossover 调和句；NTU §7 ack 语录补入正文+官方页截图落档+留档域名勘误。commits 52c1b14→9a32f9c→本批。 |
| v2.6 | 2026-09-07 | **PSD-NTU-TRANS-001 真域转换收官（R22b#5 修复落地，FAILS 如实入文）**：冻结 v1.0 跑后——**成本端点双档 FAILS**（D 冻结 pretext+CPU LR 头 466s median vs C 端到端 MLP 35s，解耦反贵 13×；10% 档 8.6 vs 3.8s 反贵 2.3×——MLP 尺度下耦合臂 35s 全重训，无成本可省+E6 缩放警示语印证）；**精度端点预注册预测双向命中**（D 全档 +3.9pp/10% 档 +11.3pp，均出 ±2.3pp 带）；判据裁决 ≥3× 主张维持合成层限定（L12 重写+正文 E6-real 段新装）；工件字段名反转（median_ratio_D_over_C 实存 C/D）如实披露不改档；solver 族未冻结如实披露不做 post-hoc 重跑。证据 `reports/r22-ntu-transition-2026-09-07.json`；协议 §8 Results。 |
| v2.5 | 2026-09-07 | **E9 NTU60 10-seed 终口径轮（R22b#1 修复落地）**：b 臂按 E9 系列惯例 3→10 seeds（判据/臂/预算/子集零改动，独立驱动不改冻结脚本）——**67.53%±0.24 → 保留率 90.7% GENERALIZES 维持**（距 90% 线 +0.7pp，3-seed 初读 90.6% 未翻转）；伪标签迭代 +1.48pp 正式检验入文（t p<1e-6/Wilcoxon p=0.002，10/10 种子）；(a)/(c) 确定性臂与 09-05 工件逐位一致自检过；协议 §8 Amendment；正文 9 处+表4+fig5 数据源切换；cover letter/台账同步；PDF 90.6 零残留。证据 `reports/r16-ntu-pseudo-10seed-2026-09-07.json`。同轮：PSD-NTU-TRANS-001 冻结 v1.0+驱动落盘（R22b#5 真域转换，用户批准）。 |
