...
> Owner: `docs/paper/experiment-skeleton.md` · W5 窗口 2026-08-23 · 状态: **终稿候选 v0.9（2026-08-26 W36 终填轮·二轮增补）**——E1-E4 数字在档；E5 负结果如实入册且 C7 换轨定稿；E6 成本 6.07× 背书 ≥3×；**tab3 六行中五行已映射溯源（W38 滑窗第三臂入表）、一行执行中（W39 预训练消融，数字未落盘）**；§4.4 NTU 行 R4：joint 单流 74.30%（Δ−0.04pp PASS_BAND 保真实证）、预注册三流线 NOT_TRIGGERED、补全链按用户裁决 A 执行中
...
|------|------------|-----------|------------------------|
| − 自监督预训练 | kNN 掉回随机水平附近 | C3 | ⏳ **PENDING（W39 full 档执行中，2026-08-26 00:25 点火：两臂 scratch/warm × 3 seeds × 50ep；数字未落盘不编造）**——W31 适配器+TDD+CPU 冒烟已交付（`reports/w31-ablation-pretrain-smoke-2026-08-25.json/md`，冒烟数字非科学对照不入表）；执行结果落 `reports/ablation-pretrain-*` 后按 quickref E1 同源协议回填本行 |
...
- [x] ~~所有 `[PENDING]` 数字待 P0.2-P0.5 报告归档后回填~~ ✅ W36 终填完成（2026-08-25/26 两轮）——可填占位符全部按索引表溯源终填
- [ ] tab3 −自监督预训练行：W39 full 档执行中，`reports/ablation-pretrain-*` 落盘后回填
