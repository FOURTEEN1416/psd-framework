...
| **实现正确性验证**✅已批准（ADR 0002） | 在 NTU60 上复现 AimCLR 原文参照成绩（79.18% 口径待核），证明本仓适配实现与官方等价后再用于动物域——重实现类论文的标配防线（风险登记册 R4）。**⚠️ 执行前置：本仓无 NTU 数据，需先下载 NTU60 骨架版（公开约 6GB 量级）+ 官方预处理；排程在 P0.2 救援释放 GPU 后开独立小窗口** | 📋 已纳入范围，待排程执行 |
