...
> Owner: `docs/paper/experiment-skeleton.md` · W5 窗口 2026-08-23 · 状态: **终稿候选 v1.0（2026-08-26 W36 终填轮·三轮收口）**——E1-E4 数字在档；E5 负结果如实入册且 C7 换轨定稿；E6 成本 6.07× 背书 ≥3×；**tab3 六行全部映射溯源完成（W38 滑窗臂 + W39 预训练消融双双入表，零 PENDING）**；§4.4 NTU 行 R4：joint 单流 74.30%（Δ−0.04pp PASS_BAND 保真实证）、预注册三流线 NOT_TRIGGERED、补全链按用户裁决 A 执行中待二次收编
...
|------|------------|-----------|------------------------|
| − 自监督预训练 | kNN 掉回随机水平附近 | C3 | ✅ **可填（合成层 full 档，W39 执行 2026-08-26，W36 入表）**：warm-init vs scratch 下游精度 **Δ=+0.15pp（96.97%±0.28 vs 96.82%±0.37，n=3 seeds，同 seed 配对 2胜1平，n.s.）**——合成层饱和档位（两臂均 >96%）下预训练对微调终点精度无可分辨贡献；收敛动力学方向一致占优（best_epoch 15.0±4.0 vs 21.7±12.9）。预训练价值主张锚定 **kNN 表征质量（P0.1：20.89%=2.51×随机）** 与解耦架构组件依赖，非下游精度增幅。【合成层 / 指标；warm 权重来源公开真实层 P0.1——两层口径分列】来源：`reports/ablation-pretrain-2026-08-25.md` §2–§4 + `reports/w31-ablation-pretrain-2026-08-26.json`。⚠️ 引用纪律（W39 §5 随行）：本行不得改写为"预训练提升/降低下游精度"；C3 原假设（kNN 掉回随机）仍属未直接检验（本实验无 scratch-encoder kNN 对照臂），如需闭环须补该臂；低资源档位（samples_per_class ∈ {5,10,20} 梯度）是否补跑留协调者/用户裁决 |
...
- [x] ~~tab3 −自监督预训练行~~ ✅ W39 full 档落地入表（2026-08-26，零结果如实 + 引用纪律随行）
