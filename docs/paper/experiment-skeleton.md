...
> Owner: `docs/paper/experiment-skeleton.md` · W5 窗口 2026-08-23 · 状态: **终稿候选 v0.8（2026-08-25 W36 终填轮）**——E1-E4 数字在档；E5 负结果如实入册且 C7 换轨定稿；E6 成本 6.07× 背书 ≥3×；tab3 六行中四行已映射溯源、两行 PENDING（W38/W39 实验窗未执行，不编造）；§4.4 NTU 行（R4）基建就绪、线性评估执行数未产出——**全文档零可填未填占位符，剩余 PENDING 均系实验未执行而非素材缺失**
...
| **实现正确性验证**✅已批准（ADR 0002） | 在 NTU60 上复现 AimCLR 参照成绩（口径已于 W9 核实：NTU60 xsub 线性评估 + 三流融合；论文正文 78.9%，79.18% 为官方 released-model 复测——`reports/ntu-phasea-2026-08-24.md`；预注册通过线维持 ≥77.18%），证明本仓适配实现与官方等价后再用于动物域——重实现类论文的标配防线（风险登记册 R4）。**⚠️ 执行前置：本仓无 NTU 数据，Phase A 数据获取受阻于网络（GDrive 本机不可达，百度云镜像需账号交互），渠道裁决待用户（选项见报告 §4.3）；预处理版实际 ≈2GB** | 🔄 **W36 状态终填（2026-08-25，事实版）**——Phase A ✅（协议核实 + 数据就位，xsub/xview 全量 verify PASS 113,156 样本）；Phase B joint 流 pretext **300/300 收官**（loss 16.63→5.30 零 NaN，`runs/ntu_phaseB/joint_pretext/epoch300_model.pt` 落盘）；线性评估基建 ✅ TDD 双提交（官方 LE_Processor 直连 + 配置保真护栏 43 测试绿）。**执行数未产出**：joint 单流线性评估（~3-5h GPU）被 relay 门禁阻塞后尚未占卡；且 W33 发现协议缺口——预注册线 ≥77.18% 的对照物是**三流融合**（README: joint 74.34/bone 71.87/motion 68.68/3s 79.18），本仓仅有 joint checkpoint，补全三流需 ~48h+ GPU 且范围裁决待用户。**数字区全部 [待执行]，成稿禁引任何未落盘线性评估数**。证据：`reports/ntu-phaseB-lineareval-2026-08-25.md`（DRAFT v0.1）+ `reports/ntu-phaseB-results.json` |
...
|------|------------|-----------|------------------------|
| − 自监督预训练 | kNN 掉回随机水平附近 | C3 | ⏳ **PENDING（W36 复核：full 档实验未执行，不编造）**——W31 已交付适配器+TDD+CPU 冒烟（真实 P0.1 ckpt strict 加载打通、同切分编排，`reports/w31-ablation-pretrain-smoke-2026-08-25.json/md`），**冒烟数字非科学对照不得入表**；full 档（两臂×3 seeds×完整预算，`configs/ablation_pretrain.yaml`）排 W33 线性评估之后（W39 窗口已注册未开工）。素材落地后本行按 quickref E1 同源协议回填 |
| − 无监督分割 | 无法处理连续流 → 只评片段级 | C4 | ⏳ **PENDING（W34 排查定案：现有 P0.2 对照不构成本消融证据，不编造；W38 实验窗已注册未开工）**——P0.2 的对照是 SMQ vs 等段数随机切分 null 基线（度量校准用途），非「有/无无监督分割」方法臂；其拼接协议负结果（SMQ 0.283 不超随机 ~0.301）是评估协议盲区披露，反向挪用会打脸 C4 主张。**所需新实验设计（W34 入册，W38 落地）**：① 最小版（纯 CPU，约半天）：seeds 伪 GT 协议下加「固定均匀滑窗」第三臂（stride=patch_size 或等段数 uniform cut），复用 `eval_smq_segmentation.py --gt-protocol seeds` 基建，判据 = 均匀窗显著劣于 SMQ 且 ≥ 随机 null 才构成边界增益消融；② 完整版（GPU 排队）：同骨干识别器分别消费自适应段 vs 固定窗的连续流输入，比端到端精度 + 跨边界动作样本的处理损失（呼应 W30 时序三选一定案）。 |
...
- [x] ~~所有 `[PENDING]` 数字待 P0.2-P0.5 报告归档后回填~~ ✅ W36 终填完成（2026-08-25）——可填占位符全部按索引表溯源终填；剩余 PENDING（tab3 两行 / NTU 执行数）均系实验未执行，非素材缺失
- [ ] tab3 −自监督预训练行：W39 full 档执行后回填；tab3 −无监督分割行：W38 滑窗第三臂落地后回填
- [ ] NTU 线性评估执行数 + 三流范围裁决（用户）后更新 §4.4 实现正确性行与 R4
