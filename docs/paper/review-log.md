...
## R17 回归审计轮（2026-09-05，Reviewer-2 回归攻击 R16 修复文本 + 主题审计/anti-defensive 双 agent 并行；修复轮自身为幻觉高发区）

**新 CRITICAL（评估器类索引 bug 传播）**：归档 p07/p12 的 `evaluate()` 以 `range(len(class_names))` 遍历非连续 int 标签（v1 {0,3..10}、v2 {3..10}），越界类被丢弃、缺席类计零——**"EP3 macro-F1 反向（14.7→7.8）"与"微调对照 macro-F1 更高（16.73 vs 7.83）"两条 R8 时代披露均为 bug 产物**。修正评估器下 v1→v2 full macro-F1=16.5%→19.3%（与 top-1 同向上升）、frozen 19.33>finetuned 16.73（双指标同向，"design superiority 仅限双指标一致区"的让步句随之简化）。r16 工件的 evaluate_str 本就正确，故本轮修正只动叙述与归档勘误（p12 评估器注记+p07 勘误文件），不改任何 r16 数字。
**MAJOR 修复**：①Holm 族口径统一为逐层（m=6，与 §4.2"within each experiment family"一致；工件重算回写：v1 mf1 spc2 0.043→**0.023**、spc4 0.098→**0.049**（升为显著，E7"only"改"two effects both favoring generic SSL"）、v2 scratch-mf1 0.024→**0.012**；工件 families 元数据与计算矛盾已修）；②E7 停止规则理由误写"requires a labeled reference"（该层标签明明存在）→改"consensus reference 不存在，用真标签即 oracle 泄漏"；③03 承诺"experiments disclose gate inertness"未兑现→E7/E9 各补半句（AK/NTU 类空间无 standing，门恒惰性，池过滤退化为纯置信边际）；④E7b 微调收尾句自相矛盾（把双指标分歧区称为一致区）→重写；⑤v2 种子数 16→**14/256≈5.5%**（sit 无训练段，"matched absolute budget"措辞随之修正，fig5 caption 13%/6%→13%/5.5%）；⑥chance 口径调和（名义 8.33→12.5 用于预注册混淆论证，实现 9 类 11.1 用于 E7，两处显式区分）；⑦E9 补 linear-only 臂 88.7% 保留语境（防"90.6% 只比平凡线高 1.9pp"的仓库级指控）；⑧05"untested"与自家工件矛盾→改"post-hoc 诊断显示伪标签质量与监督特征质量解离（0.11 warm vs 0.15 SSL），非受控检验"；⑨摘要 tier 混排（"On public real..."统辖 synthetic 结果）→拆句；⑩intro bullet4 无落点的"cold-start near chance"句删（W23 comparability 护栏），82% 主张补 §4.3 独立段落（此前仅存在于 tab2/摘要——"每实验先声明 claim"纪律的静默违例）；⑪结论补 90.6%+犬科负边界一句（Q1"这论文还剩什么"的正面回答）；⑫五臂 macro-F1 12-slot 口径披露；⑬errata 范围 60–99%→31–100%（scratch 臂池覆盖全训练集）；⑭p07 勘误文件+holm-p04 生成脚本入复现链。
**MINOR 修复**：head_calib 范围 9–12%→8.9–12.5%（三 seed）；novelty"ten groups plus a full scan"双重计数→"the last a full scan"；03"grows complete labeled coverage"→toward completeness；L1 指针错位（共识口径≠L1）→inline；"near chance"对 15.2% 臂→"at or near chance"；"reported"→"obtained"（从未发表）；内部轮次标记"(R16)"出正文+统一脚注（首次投稿不应引用未发表的内部历史）；C2 标签去向注；E9 段头补 C6 标签；04:8 K9 三重述删一；main.tex 注释 stale（十条→十一条）；fig5 caption 措辞。
...
## P1 标签对齐伪标签修正实验线（2026-09-05，ADR 0007 + PSD-ALIGN-PREREG-001，结果 NULL）

用户裁决"立项冲/强化卖点必须"→ 针对 R16 诊断的头路过拟合确认偏置坍缩，设计三个 GT 无关标签对齐变体（V1 全类锚点共识门/V2 配额/V3 原型路主导）+ V0 对照，**协议跑前冻结**（commit e814368 早于结果工件）。
**结果**：AK v1 spc2×10seeds，最优 V2=14.11%±6.65（对 V0 9.82% 配对胜 8/10、+4.29pp、p=0.13），V3 池精度 0.21 vs V0 0.11（翻倍）——机制方向有效但未达 20% 救援线，**判据 NULL**。
**纪律**：冒烟 seed42 V1=19.6% 是幸运种子，全量后回落——**未据单种子挪门柱**（这正是 R16 要杀的 opportunism）。
**处置**：负边界维持；V0-V3 入 §5 方法学消融段（"标签对齐力改善伪标签质量且 8/10 方向占优，但本层约束是种子锚定特征几何本身，非门控规则"）；无救援主张。180 training 测全绿（gate_mode 默认 standing 向后兼容）。
**教训**：预注册的价值=当冒烟种子诱人时仍按冻结判据裁决；NULL 本身是可发表的方法学证据（把"为什么不 work"从辩解升级为受控实验结论）。


---

## P2/P3 并行实验线（2026-09-05，ADR 0008，两预注册判据，结果 NULL/膝点未达）

用户"同步进行"两线 + 追问"是否胎死腹中"。判定：论文未死（脊柱 C1/C3/C4/C5 + NTU 90.6% 跨域存活，35-45% 高于期刊基线）。
- **P2 APTv2 域自适应预训练骨干（PSD-DAP-PREREG-001）NULL**：保真自检触发灾难遗忘（全监督 33.93→25%<30% 线，InfoNCE 塌 0.15），V2+DAP 11.43%<P1-V2 14.11%（胜 3/10）。佐证协议 §2 先验：APTv2→AK 拓扑/外观失配瓶颈，非适应强度。
- **P3 种子预算曲线（PSD-BUDGET-PREREG-001）膝点未达**：V2 spc2→spc12（13%→77% 标注）仅 14.11%→22.14%，全程 <24% 膝点；池伪标签净损害最终头。约束=数据质量非预算。
**三角验证结论**：P1 门控/P2 骨干/P3 预算三独立杠杆全 NULL，一致否证"AK 层端到端低资源可救援"，把 L9 机制诊断升级为三向受控否证——比勉强救援更强的可发表负结果。纪律：两协议跑前冻结（commit bcaaaa5 早于 P2 结果），P2 冒烟未据遗忘前信号挪门柱。§5 已加 P2/P3 合并消融段。
**教训**：①强化卖点须接受"卖点可能确实不成立"的证伪风险，预注册的价值正在此；②保真自检（DAP 全监督参照下限）是表征适配实验的必备防线，否则会把灾难遗忘误读为"方法无效"；③负结果的三角验证（多杠杆同向否证）是顶刊级的诚实资产，不是减分项。


---

## P4 公开数据调研 + 选项2 动机重写（2026-09-05，用户裁决"选2"）

**工具纠偏**：用户指出我 firecrawl 限流后只靠 API 直查是"死守单一工具"（违反搜索分域）。改用 crawl4ai（crwl 爬 SERP）+ anysearch（学术/通用/资源垂直域 + extract 官方页）穷尽检索——**纠正了我"公开检索基本空白"的草率结论**（确实有一批具名数据集），但穷尽后结论更硬。
**P4 结论（全部官方页实证）**：无任何公开数据集提供"真实工作犬+干净骨架+专家逐段行为标签"到基准规模。地形：PawCraft 证伪（查无此物，未凭记忆瞎报）；Kaggle"Dog Behavior Analysis"抽页确认=Vehkaoja 项圈 IMU 数据（视频仅标注不发布，错模态）；SyDog-Video=合成；Ultralytics Dog-Pose=24 关键点但仅图像无行为时序；Mendeley/传感器集=IMU；唯一大规模真实动物行为视频基准=Animal Kingdom（已在用，就是瓶颈源）。
**结构性洞察**：AK 瓶颈不是"选错数据集"，是真实犬行为公开数据本身稀缺——**这正是论文开篇前提"behavior annotation is scarce"的实证**。负边界不该藏，应转为立论证据。
