...
| ✅ P0.2 SMQ 时序分割 | 1 周 | 分割模型 + episode 边界 IoU 评估 + 可视化 | `scripts/eval_smq_segmentation.py --iou`（**2026-08-24 W7 救援达标：根因=官方超参致编码器码塌缩，单变量 mse_loss_weight→1.0 修复；种子伪 GT 口径 IoU 0.409 vs 随机 ~0.300（1.36×）、F1 0.418；双口径并列披露，报告 `reports/p02-2026-08-24.md`**） |
