...
| E2 | ST-GCN+BC / 21类(E6-Y') / 100样本/类 | val_acc | **95.9%** | 4.76% | 20.1× | `reports/p05-stgcnbc-synthetic-100perclass-Yprime.json` | 复核 **96.8%**（08-24 12:52 等预算 50ep · 20.3× · 当前报告内容）；同日早停版复核 **95.7%**（ep30/best ep15，报告已被等预算版覆写，数字仅存此行） |
...
>
> 🔁 **复核约定（2026-08-24）**：P0.2 / P0.5 已按下方「回填命令序列」当次重跑；「论文 skeleton 行」列标注『复核』的为重跑新鲜值，「数值」列保留首跑数字作对照。E1 覆写为同协议重跑（seed=42 固定，-0.9pp 为 GPU 非确定性波动）；E2 被并行窗口以**等预算协议**（`--no-early-stopping`，50ep 满跑）二次覆写，早停/等预算两种口径数字均记录于行内，引用时须注明协议；E-C 首跑报告保留不动，新鲜值落 `-recheck.json`。结论均不变：各值远超随机基线×3 判据线；E-C 与首跑逐位一致。
>
> 🛡️ 运维注记：首份 E-C recheck 文件（12:30 写入）曾被并行执行体删除（疑似 12:37 计划任务 `\OpenCode\psd-overnight-supervisor` 实例偏离任务书），已确定性重跑再生并立即入库。另：本窗口的 E-C recheck 与 E2 报告曾被并行窗口提交 `8f0c293` 卷入入库（内容经本窗口核验无误，特此声明归属）。建议晨会核查该计划任务与多窗口的写权限/提交边界。
>
> 🧹 历史修正（2026-08-24 15:4x）：P0.4 消融窗口的失实提交 `b1a4323`（信息声称含 p04 产物实际不含，且卷入 6 个无关文件）已按语义三层重建为 `5f06663`(feat p04 消融) → `389bb49`(fix 主配置识别回归) → `a52b3a1`(chore p03/p05 并行产物归位)；合法路径树内容经 `git diff backup/pre-rewrite-20260824 HEAD` 逐位核验一致，杂物剥离至工作区 untracked，`.playwright-mcp/` 已入 .gitignore。旧链保留于备份分支 `backup/pre-rewrite-20260824` 供审计，确认无用后可删。
