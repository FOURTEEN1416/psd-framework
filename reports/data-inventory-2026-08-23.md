...
1. ~~「338/239」「242K」需 K9 侧口径确认~~ → 已闭环，见 §6
2. ~~`project-brief.md` §8「数据集磁盘路径盘点」待确认项建议由收尾会话勾销~~ → 用户已授权超额完成，已勾销（同日）

## 6. 后续处置记录（2026-08-23 同日追加，用户双轨裁决）

1. **溯源结论**：「338 视频 / 239 帧级标注」「242K 文件」均由 K9 仓 commit `9f63494`（2026-08-20 P0 立项规划）写入 `stages/data-unblocking.md` 与 `stage-plan.md`；全仓检索（md/py/txt/json/ipynb/yaml）+ `git log -S` 确认**无任何统计脚本、报告或命令留痕**——判定为**规划期未验证估计**。
2. **K9 truth 回改**（commit `faaab28`）：AK 犬科 → 329 视频 / 34,772 帧行；APTv2 → 83,304 文件；stage-plan 升版 v1.13 记录修正。
3. **本仓标注保留并升级**：差异标注由「需 K9 口径确认」改为「已溯源修正」状态（DATA_LOCATIONS.md / project-brief.md §4 表格同步实测值、§8 盘点项勾销）。
4. 溯源附带发现：K9 `reports/phase-3.2d-multi-dog-tracking-validation.md` 中犬科计数（290 帧 / 852 条标注）属 APTv2 YOLO 微调线产物，与本报告 AK 复核无关，不冲突。
