...
## 七、R43b 顶刊级冲刺（同日第二轮：v13/v16/v8/v6/v6/v13）

用户追问"能否更接近顶刊级"后，对照 Nature/Cell 系图表语法做差距诊断并落地五类升级，
全部为增量精修（拓扑/数据/色板/caption 仍零改动）：

| 图 | 版本 | 顶刊化升级 |
|----|------|-----------|
| fig1 | v13 | 双层容器加同相降饱和发丝边框（diagram-design "borders are in"） |
| fig2 | v16 | 自动环四步 ①-④ 编号 chip（悬浮盒顶，与 caption "left to right" 叙述序对应） |
| fig3 | v8 | Nature 惯例面板字母 a/b/c（左上粗体小写、轴外）；网格减淡；轨道加高 |
| fig4 | v6 | 网格 y-only 减淡；笔触系统性调细一档（轴 0.65/marker 5.6/误差棒 0.9） |
| fig5 | v6 | **直接标注替换远端图例**（P8 直标优于图例；顺带消解 7 类目 WARN）；100% 线注记 "own full budget"；画布 2.75in |
| fig_ga | v13 | 保留率条垫 0-100% 基准轨+右端 "100%" 刻度（条长获得可读比例尺） |

**fig5 直标的严格验证（R24"程序化复核"教训的制度化）**：
- 标签落位先做几何预计算（避开自家/邻点 whisker 竖线与 marker）；
- 脚本内建**三重程序化碰撞检测**：①标签两两 bbox 相交断言 ②出轴断言
  ③**文本×误差棒竖线相交断言**（渲染坐标变换后判定）——③曾抓出 300dpi zoom
  才发现的 "public-real v1" 居中位被自家 ±22.1pp 长 whisker 穿过，右移绕行后
  复检 PASS（7 标签两两净空+不出轴+零穿线）。

**就地迭代 2 处**：fig2 chip 首版被左缘裁切且 ② 压住 "κ ≥ τ" 标签 → chip 悬浮盒顶
内收 1.2 + 标签 ldy 2.6→1.6；GA v13 基准轨引用了后定义的 BAR_H → 字面值修复。

**验收（复跑全绿）**：pdflatex ×2 → 0 错误 0 浮动警告 42 页；落位 p4/10/15/20/31
ALL OK；?? = 0；USER = 0；fig5 碰撞检测 PASS；300dpi zoom 双核确认直标绑定清晰。

**顶刊清单遗留项（有意不做，留痕）**：fig1/fig2 卡片内微图标——单栏 3.42in 下卡片仅
0.6in 宽，图标会退化为噪点（带图标的方法图惯例是双栏 7in 版面），待期刊排版确定
双栏可行性再评估。
