...
> 日期: 2026-08-24 | 窗口: W6 | 状态: ✅ 完成（P0.3 锚点备料；同日独立复核优化后 v2 重生成）
...
> 版本: v2（初版 aaa1e1c；复核修复 I-1/I-2 数值伪迹后全量重生成，见 §5 缺陷 5/6）
...
### 4.1 数据集帧占比（公开真实层口径，v2 重生成）

| 类别 | 占比 | 备注 |
|------|------|------|
| sitting | 37.5% | 人犬互动场景主态 |
| walking | 22.1% | |
| standing | 21.8% | |
| running | 6.2% | 室内真跑少数，合理 |
| rise_transition | 5.7% | 偶发事件（v2 已剔除缺口伪迹，见 §5 缺陷 5）|
| jump | 4.9% | 含真实跳跃与残余误判，P0.3 置信度过滤可压 |
| lying | 1.7% | **新鲜证据**：该池犬只少卧姿；旧调研「以卧为主」结论源自 kpm 不同数据池，不适用本池 |
| unknown | 0.1% | |
...
| 规则 ID | 帧覆盖* | | 质量指标 | 数值 |
|---|---|---|---|---|
| gait_walk | 57.1% | | clip 平均置信度 p10/p50/p90 | 0.735 / 0.801 / 0.873 |
| sitting_posture | 50.5% | | 高置信 clip (≥0.7) | **96.9%** |
| standing_posture | 43.7% | | 种子段总数（v2） | 4,708 |
| gait_run | 13.8% | | 段数/clip 中位 / p90 / max | 19 / 34 / 66 |
| jump_airborne | 12.2%† | | 有效 clip | 225/226（剔除 1 个全 NaN，同 P0.1 口径）|
| rise_up / lie_down | 7.6% / 7.5% | | NPZ 字段截断段 | 0 |
| lying_posture | 2.9% | | | |
...
| 4 | 规则 ID 存储截断（`standing_postur`） | NPZ 结构化数组 rules 字段 U64 < 最长联合串 66 字符 | U128（v2 起宽度由内容派生）；复测截断段数=0 |
| 5 | **I-1**：NaN 置零先于平滑/差分，缺口邻域被拖向地面 → 伪 rise_transition 段（审查实证：静立犬产出 conf=0.318 的"卧下又站起"矛盾段） | `nan_to_num(0)` 在 kp 与 clearance 两处先于 smooth/gradient 执行 | 新增 `_interp_nan_time` 时间轴线性插值，kp/clearance 先填缺口再差分；回归测试 `test_nan_gap_produces_no_fake_transition`；真实数据验证：20 clip 挖 8 帧缺口后过渡段 65→63（无批量泄漏）|
| 6 | **I-2**：质心按"当帧有效关节子集"计算，遮挡使子集系统性变化 → 质心跳变 → 伪 running（审查实证：头簇失效后半段立刻出现伪跑） | 步态质心的关节集合逐帧漂移 | 改用固定躯干子集 {2,5,8,11,22}（§2 实证长期稳定）+ 子集缺失帧插值；回归测试 `test_head_occlusion_no_fake_gait` |

### 5.1 独立复核记录（code-reviewer 子代理，2026-08-24）

审查范围 `6f67e88..aaa1e1c`。结论：无 Critical；2 Important（上表缺陷 5/6，均已实证复现→修复→回归锁定→全量重生成）；10 Minor 中 8 项已顺手修复（标签宽度派生、测试配置漂移、陈旧注释、热循环常量提升、config_echo 全量快照入库、frame_idx 单调性守卫、口径分叉 docstring 备案），2 项备案为已知取舍（见 §8）。性能实测 ≈5.3ms/clip，引擎侧全量 ≈1.2s，无需向量化。
...
- [未验证] 段粒度较碎（v2 中位 19 段/clip），下游锚点抽取建议叠加最短持续时长过滤（≥0.5s）
...
- [设计取舍备案] transition 优先级高于 gait（真跳跃起跳段会被标过渡）；unknown 短段并入高置信邻段（正标签扩张）；双向符号（rise_up+lie_down）共存于一段多为合法复合动作（起立前重心调整），非缺口伪迹——I-1 修复已用"挖缺口对照实验"（65→63）甄别确认
- [流程备注] 交接文档所写测试路径 `psd/data/test_rule_seeds.py` 实际落位 `psd/data/tests/test_rule_seeds.py`（沿用仓库既有 tests/ 约定），后续窗口按实际路径查找
